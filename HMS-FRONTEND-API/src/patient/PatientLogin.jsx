...
export default function PatientLogin() 
{
  ...
  const { setIsPatientLoggedIn, setPatientData } = useAuth();
  ...
  const handleSubmit = async (e) => 
  {
    ...
    catch (error) 
    {
      if (error.response) 
      {
        ...
        {
          setError("Patient not found. Please check your credentials.");
        }
        else 
        {
    ...
    <div className="patient-container">
      <div className="patient-form">
        ...
        <div className="text-center mt-3">
          <p>Don't have an account? <Link to="/patientregister" className="text-link">Register Here</Link></p>
        </div>
      </div>
    </div>
  );
